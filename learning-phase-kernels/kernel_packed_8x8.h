...
#define  micro_kernel_8x8\
    ...
    b0 = _mm256_set1_epi32((int32_t)B(j,  k));\
    b1 = _mm256_set1_epi32((int32_t)B(j+1,k));\
    b2 = _mm256_set1_epi32((int32_t)B(j+2,k));\
    b3 = _mm256_set1_epi32((int32_t)B(j+3,k));\
    b4 = _mm256_set1_epi32((int32_t)B(j+4,k));\
    b5 = _mm256_set1_epi32((int32_t)B(j+5,k));\
    b6 = _mm256_set1_epi32((int32_t)B(j+6,k));\
    b7 = _mm256_set1_epi32((int32_t)B(j+7,k));\
...
static int8_t Buffer_A[MC * KC] __attribute__((aligned(64)));
static int8_t Buffer_B[NC * KC] __attribute__((aligned(64)));
...
void pack_A(int8_t* A, int8_t* Buffer_A, int mc, int kc, int row_start, int col_start, int LDA) {
    for (int j = 0; j < mc; j += 8) {
        ...
        for (int p = 0; p < kc; p++) {
            for (int i = 0; i < mr; i++)
                *Buffer_A++ = A(row_start + j + i, col_start + p);
            for (int i = mr; i < 8; i++)
                *Buffer_A++ = 0;
        }
    }
...
void pack_B(int8_t* B, int8_t* Buffer_B, int nc, int kc, int col_start, int row_start, int LDB) {
    for (int j = 0; j < nc; j += 8) {
        int nr = min(8, nc - j);
        for (int p = 0; p < kc; p++) {
            for (int i = 0; i < nr; i++)
                *Buffer_B++ = B(row_start + p, col_start + j + i);
            for (int i = nr; i < 8; i++)
                *Buffer_B++ = 0;
        }
    }
...
void padding(int32_t M, int32_t N, int32_t K, int8_t *A, int LDA, int8_t *B, int LDB, int32_t *C, int LDC){
    ...
    for (i=0;i<M;i++){
        for (j=0;j<N;j++){
            int32_t acc = C(i,j);
            for (k=0;k<K;k++){
                acc+= (int32_t)A[k*8 + i] * (int32_t)B[k*8 + j];
            }
            C(i,j) = acc;
        }
    }
 ...
 void macro_kernel(int32_t M,int32_t N,int32_t K,int8_t* A,int LDA, int8_t* B,int LDB, int32_t* C,int LDC){
    int i, j, k;
    __m128i a8;
    __m256i a,a16,b0, b1, b2, b3, b4, b5, b6, b7, c0, c1, c2, c3, c4, c5, c6, c7;
    int M8=M&-8;
    int N8=N&-8;
    int K8=K&-8;
 ...
 void kernel(int32_t M,int32_t N,int32_t K,int8_t* A,int LDA, int8_t* B,int LDB, int32_t* C,int LDC){
    ...
    for (int j = 0; j < N; j += NC) {
        ...
        for(int p = 0; p < K; p += KC) {
            int kc = min(K-p,KC);
            pack_B(B, Buffer_B, nc, kc, j, p, LDB);
            for(int i = 0; i < M; i += MC) {
                ...
                for(int jr = 0; jr < nc; jr += 8) {
                    ...
                    for(int ir = 0; ir < mc; ir += 8) {
                       ...
                       macro_kernel(mr, nr, kc, &Buffer_A[ir*kc], 8, &Buffer_B[jr*kc], 8, &C(i+ir,j+jr), LDC);
